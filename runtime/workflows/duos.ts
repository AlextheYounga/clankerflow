...
async function runPlanner(tools: WorkflowTools) {
  ...
  const result = await tools.agent.run({ title: "Planner: Design and create tickets", prompt });
...
async function runBuilder(tools: WorkflowTools, ticket: Ticket) {
  ...
  const result = await tools.agent.run({ title: `Builder: ${ticket.title}`, prompt });
  if (!result.ok) throw new Error(`Builder agent failed on ticket ${ticket.ticketId}: ${result.error}`);
}

async function buildTicket(tools: WorkflowTools, initial: Ticket) {
  await tools.tickets.updateStatus({ id: initial.ticketId, status: "IN_PROGRESS" });
  ...
  for (let cycle = 1; cycle <= MAX_REVIEW_CYCLES; cycle++) {
    ...
    if (ticket.status !== "QA_CHANGES_REQUESTED") {
      tools.log.warn(`Ticket ${ticket.ticketId} has unexpected status '${ticket.status}' — stopping`);
  ...
  tools.log.warn(`Ticket ${ticket.ticketId} unresolved after ${MAX_REVIEW_CYCLES} cycle(s)`);
  return { ticketId: ticket.ticketId, cycles: MAX_REVIEW_CYCLES, ok: false, status: "QA_CHANGES_REQUESTED" };
}

export default async function duosWorkflow(context: WorkflowContext, tools: WorkflowTools) {
  // Shortcut: if a ticket was passed directly, skip planning
  if (context.ticket) {
    tools.log.info(`Ticket provided — skipping planner, building ticket ${context.ticket.ticketId}`);
