...
/// # Errors
/// Returns an error if the executable path cannot be resolved or the bundled
/// Node runtime is missing and `AGENTCTL_NODE_BIN` is not set.
pub fn resolve_node_bin() -> Result<PathBuf> {
    let exe = env::current_exe().map_err(|e| anyhow!("failed to resolve executable: {e}"))?;
    let exe_dir = exe
        .parent()
        .ok_or_else(|| anyhow!("failed to resolve executable directory"))?;
    resolve_node_bin_from(exe_dir)
}

/// # Errors
/// Returns an error if the bundled Node runtime is missing and
/// `AGENTCTL_NODE_BIN` is not set.
pub fn resolve_node_bin_from(exe_dir: &Path) -> Result<PathBuf> {
    if let Ok(override_bin) = env::var("AGENTCTL_NODE_BIN") {
...
mod tests {
    ...
    fn env_override_is_respected() {
        ...
        unsafe { env::set_var("AGENTCTL_NODE_BIN", "/usr/bin/node") };

        let result = resolve_node_bin_from(&PathBuf::from("/tmp/agentctl-test"));

        unsafe { env::remove_var("AGENTCTL_NODE_BIN") };
