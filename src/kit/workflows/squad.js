...
async function runProjectManager(ctx, outlineContent) {
...
async function runDev(ctx, ticket) {
...
async function runQA(ctx, ticket, devOutput) {
...
async function processDevQaCycle(ctx, ticket) {
  assertOk(
...
async function passTicketToDevTeam(ctx, ticket) {
  for (let cycle = 1; cycle <= MAX_REVIEW_CYCLES; cycle++) {
    const updatedTicket = await processDevQaCycle(ctx, ticket)
    ...
    if (status !== 'QA_CHANGES_REQUESTED') {
      ctx.log.warn(
        `Ticket ${ticket.ticketId} has unexpected status '${status}' after QA — stopping`
      )
      return { ticketId: ticket.ticketId, cycles: cycle, ok: false, status }
    }
...
export default async function squadWorkflow(ctx) {
