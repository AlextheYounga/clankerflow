...
- [x] `kit/` is the source-of-truth scaffold mirrored into `.agents/` during `clankerflow init`
- [x] `.agents/.clankerflow/` is treated as gitignored local runtime state
...
- [x] Host/container runner wiring lives in `src/core/runner/env.rs`
...
- [x] Runtime helpers implement sleep, exec, fs, git, and ticket APIs used by workflows
...
- [x] `kit/opencode.json` — minimal OpenCode config template (`$schema`, `model`)
...
- [x] Rust binds `127.0.0.1:0`, passes port via `CLANKERFLOW_IPC_PORT`, and Node connects back over TCP
- [x] `runtime/src/ipc.ts` — `IpcTransport` uses `net.createConnection`; container mode connects to `host.docker.internal`; `"error"` handler logs connection failures; `CLANKERFLOW_IPC_PORT` guarded with clear exit on missing
